...
export default {
  ...
  activate(state) {
    ...
    this.shortcutsView = new ShortcutsView({
      cancel: () => this.shortcuts(false),
      save: () => this.shortcuts(true),
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      atom.commands.add('atom-workspace', {
        'patom:shortcuts': () => this.shortcuts()
      }),
  ...
  shortcuts(save) {
    if (this.shortcutsPanel.isVisible()) {
      if (save) {
        ...
        atom.config.set('patom.shortcuts', shortcuts); // save
      }
